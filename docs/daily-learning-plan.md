...
5. 用 5 分钟更新日志并做英文口述：做了什么、为什么这样做、另一个方案及取舍。

完成标准：代码可运行、至少一项自动检查通过、当天日志能回答上述四个问题。到 120 分钟立即停止，把未完成项写入日志；下一个学习日先完成它，不通过熬夜追回进度。

### 考试优先规则

- 这 120 分钟是上限，不是必须耗满的最低时长。
- 考试前 14 天可切换到维护模式：每次 30 分钟，只复习旧代码、修一个小问题或补一个测试，不开启新阶段。
- 考试前 3 天暂停项目；考试结束后从日志中的 `Tomorrow's first action` 恢复，不重新计算天数。
